...
export default function LateralMovementModal() {
  ...
  useEffect(() => {
    ...
    const off = tle.onAnalysisProgress((p) => {
      ...
      setModal((q) => (q?.type === "lateralMovement" && q.phase === "loading"
        ? { ...q, lmProgress: Math.max(0, Math.min(99, pct)), lmPhaseLabel: p.stage || p.phase || q.lmPhaseLabel, _lmRealProgress: true }
        : q));
    });
  ...
  const cancelAnalysis = async () => {
    ...
    try {
      ...
      const mine = jobs.filter((j) => (j?.status === "running" || j?.status === "queued")
  ...
  const handleAnalyze = async (overrides = {}) => {
    ...
    try {
      ...
      if (isIpcError(result) || result?.error) {
        const msg = isIpcError(result) ? ipcErrorMessage(result) : result.error;
        setModal((p) => isCurrentRun(p) ? { ...p, phase: "config", loading: false, error: msg, lmProgress: 0 } : p);
      } else {
    ...
    >
      {({ startDrag: startLmDrag, height: lmH }) => (<>
        ...
        <div onMouseDown={startLmDrag} style={{ padding: "16px 20px 12px", borderBottom: `1px solid ${th.border}22`, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, background: `linear-gradient(135deg, ${th.panelBg}ee, ${th.modalBg}dd)`, backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", cursor: "grab" }}>
          ...
          <button onClick={() => { if (phase === "loading") cancelAnalysis(); setModal(null); }} style={{ width: 24, height: 24, borderRadius: 12, background: th.textMuted + "15", border: "none", color: th.textMuted, cursor: "pointer", fontSize: LM_TYPOGRAPHY.heading, fontFamily: "-apple-system, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", transition: "all var(--m-base)" }}
          ...
          {phase === "loading" && (() => {
            ...
            return (
              <div style={{ padding: "50px 40px 40px", textAlign: "center" }}>
                ...
                <div style={{ color: th.text, fontSize: LM_TYPOGRAPHY.heading, fontWeight: 500, marginBottom: 6, fontFamily: "-apple-system, sans-serif", letterSpacing: "-0.2px" }}>{modal.lmPhaseLabel || plabels[pi]}</div>
                    ...
                    {(() => {
                      ...
                      const exportEsCsv = () => {
                        ...
                        const lines = sorted.map(s => [
                          ...
                          (s.users || []).join("; "), (s.serviceNames || []).join("; "),
                          (s.imagePaths || []).join("; "), (s.eventActors || []).join("; "),
                          (s.serviceAccounts || []).join("; "), redactRow(s.commandLines || []).join("; "), s.findingCount, s.eventCount,
                          fmtTs(s.startTime), fmtTs(s.endTime), s.status,
                          (s.categories || []).join("; "), (s.evidencePills || []).map(p => p.text).join("; "),
                        ].map(csvEsc).join(","));
                        _saveExport(header + "\n" + lines.join("\n"), "execution-sessions.csv", [{ name: "CSV", extensions: ["csv"] }]);
                      };
                      return (
                  ...
                  return (
                    <div key={f.id} style={{ borderRadius: 6, border: `1px solid ${isExpanded ? th.accent + "55" : th.border + "66"}`, background: isExpanded ? `${th.accent}08` : th.panelBg, cursor: "pointer", transition: "border-color var(--m-base)" }}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", minHeight: 28 }}>
                        <span style={{ padding: "1px 6px", background: `${LM_SEV_COLORS[f.severity] || th.sev.info}16`, color: LM_SEV_COLORS[f.severity] || th.sev.info, border: `1px solid ${LM_SEV_COLORS[f.severity] || th.sev.info}28`, borderRadius: 3, fontSize: LM_TYPOGRAPHY.badge, fontWeight: 700, textTransform: "uppercase", fontFamily: "-apple-system, sans-serif", letterSpacing: "0.03em", flexShrink: 0 }}>{f.severity}</span>
                                ...
                                {f.relatedFindingIds.slice(0, 4).map(rid => {
                                  ...
                                  return (
                                    <span key={rid} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 7px", background: `${th.textMuted}0d`, border: `1px solid ${th.border}55`, borderRadius: 4, fontSize: LM_TYPOGRAPHY.meta, cursor: "pointer", fontFamily: "-apple-system, sans-serif" }}
                                      ...
                                      <span style={{ color: LM_SEV_COLORS[rf.severity] || th.sev.info, fontWeight: 600, textTransform: "uppercase", fontSize: LM_TYPOGRAPHY.badge }}>{rf.severity}</span>
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
                      ...
                      {Object.entries(sevCounts).filter(([, v]) => v > 0).map(([sev, cnt]) => (
                        <span key={sev} style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "2px 7px", background: `${LM_SEV_COLORS[sev] || th.sev.info}12`, color: LM_SEV_COLORS[sev] || th.sev.info, border: `1px solid ${LM_SEV_COLORS[sev] || th.sev.info}22`, borderRadius: 4, fontSize: LM_TYPOGRAPHY.meta, fontWeight: 600, fontFamily: "-apple-system, sans-serif", textTransform: "uppercase" }}>
                          {cnt} {sev}
                        </span>
                      ))}
                    </div>
                          ...
                          return (
                            <div key={inc.id} style={{ padding: "10px 12px", background: `${th.accent}06`, border: `1px solid ${th.border}66`, borderRadius: 8 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
                                <span style={{ padding: "1px 6px", background: `${LM_SEV_COLORS[inc.severity] || th.sev.info}16`, color: LM_SEV_COLORS[inc.severity] || th.sev.info, border: `1px solid ${LM_SEV_COLORS[inc.severity] || th.sev.info}28`, borderRadius: 3, fontSize: LM_TYPOGRAPHY.badge, fontWeight: 700, textTransform: "uppercase", fontFamily: "-apple-system, sans-serif" }}>{inc.severity}</span>
                              ...
                              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 8 }}>
                                {memberFindings.map(mf => (
                                  <span key={mf.id} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 7px", background: `${th.textMuted}0d`, border: `1px solid ${th.border}55`, borderRadius: 4, fontSize: LM_TYPOGRAPHY.meta, cursor: "pointer", fontFamily: "-apple-system, sans-serif" }}
                                    ...
                                    <span style={{ color: LM_SEV_COLORS[mf.severity] || th.sev.info, fontWeight: 600, textTransform: "uppercase", fontSize: LM_TYPOGRAPHY.badge }}>{mf.severity}</span>
                      ...
                      return (
                        <div style={{ maxHeight: lmH - 250, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
                          {allCampaigns.map((camp) => {
                            ...
                            const campSevColor = LM_SEV_COLORS[camp.severity] || th.sev.info;
                                ...
                                {isExp && (
                                  <div style={{ padding: "0 14px 14px", borderTop: `1px solid ${campSevColor}22` }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 10 }}>
                                      {memberIncs.map((inc) => {
                                        const incSevColor = LM_SEV_COLORS[inc.severity] || th.sev.info;
                                        return (
                    ...
                    <div style={{ display: "flex", gap: 3, marginBottom: 8, alignItems: "center", padding: "4px 6px", background: `${th.panelBg}88`, borderRadius: 8, border: `1px solid ${th.border}22` }}>
                      ...
                      <button onClick={() => {
                        const payload = {
                          exportedAt: new Date().toISOString(),
                          stats: data.stats,
                          findings: redactRow(data.findings || []),
                          edges: (data.edges || []).map((e) => ({ source: e.source, target: e.target, count: e.count, logonTypes: e.logonTypes, users: e.users, firstSeen: e.firstSeen, lastSeen: e.lastSeen, riskScore: e.riskScore, technique: e.technique })),
                      ...
                      return (
                        <div style={{ marginTop: 10, padding: 14, background: `linear-gradient(135deg, ${nc}08, ${th.panelBg}ee)`, borderRadius: 10, border: `1px solid ${nc}22`, backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                            ...
                            <button onClick={() => {
                              const modalCols = data?.columns || modal.columns || {};
                              const srcCol = modalCols.source || modalCols.workstation;
                              const tgtCol = modalCols.target;
                              if (srcCol || tgtCol) {
                                const cf = { ...(ct.columnFilters || {}) };
                                if (node?.isSource && !node?.isTarget && srcCol) cf[srcCol] = selectedNode;
                                else if (node?.isTarget && !node?.isSource && tgtCol) cf[tgtCol] = selectedNode;
                                else if (srcCol && tgtCol) { cf[srcCol] = selectedNode; cf[tgtCol] = selectedNode; }
                                else if (srcCol) cf[srcCol] = selectedNode;
